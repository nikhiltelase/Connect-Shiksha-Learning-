    ...
    <>
      <nav className="bg-gradient-to-r from-blue-500 to-indigo-500 py-4 px-16 fixed w-full top-0 z-50 shadow-lg">
        <div className="container mx-auto flex justify-between items-center">
          <Link to="/" className="text-white text-3xl font-bold hover:opacity-80">
            My Shopee
          </Link>
          <div className="flex items-center w-full max-w-xl mx-auto relative">
            <input
              ...
              className="w-full px-4 py-2 rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-300 transition duration-300"
              aria-label="Search"
            />
            <FaSearch className="absolute right-3 text-gray-500 cursor-pointer hover:text-indigo-300 transition duration-300" />
          </div>
          <div className="flex items-center gap-6">
            <Link to="/login" className="text-white text-2xl flex items-center gap-2 hover:opacity-80">
              <FaRegUserCircle />
              Login
            </Link>
            <Link to="/cart" className="text-white text-3xl relative cursor-pointer hover:opacity-80">
              <FaShoppingCart />
              {cart.length > 0 && (
