...
function CartList() {
  ...
  return (
    <>
      <Navbar />
      <div className="container w-full h-screen mb-10 p-6 bg-slate-200 flex flex-col items-center">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-8">
          {cart.length > 0 ? (
            cart.map((item, index) => (
              <div
                key={index}
                className="max-w-sm rounded overflow-hidden shadow-lg bg-white flex flex-col justify-between"
              >
                <img
        ...
        {cart.length > 0 && (
          <button className="text-3xl bg-green-400 p-3 rounded-3xl m-10">
            Buy now
          </button>
        )}
      </div>
